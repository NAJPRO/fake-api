...
class UpdateUserRequest extends FormRequest
{
    ...
    {
        $userId = $this->route('user'); // Récupère l'ID de l'utilisateur
        ...
        return [
            ...
            'email' => [
                'email',
                'required',
                Rule::unique('users', 'email')->ignore($userId), // Ignore l'email actuel
            ],
            'avatar' => 'nullable|string|max:255',
            'role' => ['required', Rule::in(['user', 'admin'])],
            'password' => ['nullable', 'min:4', 'max:255'], // Mot de passe optionnel
        ];
    }
