...
class TagRequest extends FormRequest
{
    ...
    {
        $tagId = $this->route('tags'); // Récupère l'ID de la catégorie en cas de modification

        return [
            'name' => ['required', 'string', 'max:255', 'min:2'],
            'slug' => [
                'nullable', 'string', 'max:255',
                Rule::unique('tags', 'slug')->ignore($tagId), // Ignore l'ID de la catégorie en cas de modification
            ],
